...
public class Stop implements ServerControls {
    public void execute(String serverType, List<ClientServer> servers) {
        System.out.println(Log.Color.YELLOW.getColor() + "Stopping all " + serverType + "...");
        ...
        for (ClientServer server : servers) {
            ...
            if (state == UtilizationState.STOPPING || state == UtilizationState.OFFLINE) {
                System.out.println(Log.Color.YELLOW.getColor() + server.getName() + " was skipped due to it already being stopped!");
                continue;
            }

            server.stop().execute();
            System.out.println(Log.Color.GREEN.getColor() + "Successfully stopped " + server.getName() + "!");
        }
